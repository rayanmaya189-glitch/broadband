...
async fn test_create_subscription() {
    ...
    assert!(
        result.id > 0,
        "Subscription should be created with positive ID"
    );
}
...
async fn test_subscription_lifecycle() {
    ...
    let suspended = active_model
        .update(db)
        .await
        .expect("Failed to suspend subscription");
...
async fn test_subscription_cancellation() {
    ...
    let cancelled = active_model
        .update(db)
        .await
        .expect("Failed to cancel subscription");
    assert_eq!(cancelled.status, "cancelled");
    assert!(
        cancelled.end_date.is_some(),
        "End date should be set after cancellation"
    );
}
