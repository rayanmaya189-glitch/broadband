...
fn user_routes() -> Router<SharedState> {
    Router::new()
        .route("/", axum::routing::get(crate::modules::identity::api::http::list_users))
        .route("/me", axum::routing::get(crate::modules::identity::api::http::get_current_user))
}

fn branch_routes() -> Router<SharedState> {
    use crate::modules::branches::api::http;
    Router::new()
        .route("/", axum::routing::get(http::list_branches).post(http::create_branch))
        .route("/:id", axum::routing::get(http::get_branch).put(http::update_branch).delete(http::delete_branch))
}

fn customer_routes() -> Router<SharedState> {
    use crate::modules::customer::api::http;
    Router::new()
        .route("/", axum::routing::get(http::list_customers).post(http::create_customer))
        .route("/:id", axum::routing::get(http::get_customer))
        .route("/:id/status", axum::routing::put(http::update_customer_status))
        .route("/:id/addresses", axum::routing::get(http::list_addresses).post(http::add_address))
...
fn subscription_routes() -> Router<SharedState> {
    ...
    Router::new()
        .route("/", axum::routing::get(http::list_subscriptions).post(http::create_subscription))
...
fn billing_routes() -> Router<SharedState> {
    ...
    Router::new()
        .route("/invoices", axum::routing::get(http::list_invoices).post(http::create_invoice))
        .route("/payments", axum::routing::get(http::list_payments).post(http::record_payment))
}
...
fn rbac_routes() -> Router<SharedState> {
    ...
    Router::new()
        ...
        .route("/users/:id/roles/:role_id", axum::routing::delete(http::revoke_role))
}

fn accounting_routes() -> Router<SharedState> {
    use crate::modules::accounting::api::http;
    Router::new()
        .route("/accounts", axum::routing::get(http::list_accounts))
}

fn network_routes() -> Router<SharedState> {
    use crate::modules::network::api::http;
    Router::new()
        .route("/vlans", axum::routing::get(http::list_vlans).post(http::create_vlan))
        .route("/vlans/:id", axum::routing::delete(http::delete_vlan))
        .route("/ip-pools", axum::routing::get(http::list_ip_pools).post(http::create_ip_pool))
        .route("/pppoe/sessions", axum::routing::get(http::list_pppoe_sessions).post(http::create_pppoe_session))
        .route("/pppoe/sessions/:id/terminate", axum::routing::post(http::terminate_pppoe_session))
        .route("/mac-bindings", axum::routing::get(http::list_mac_bindings).post(http::create_mac_binding))
}

fn device_routes() -> Router<SharedState> {
    use crate::modules::device::api::http;
    Router::new()
        .route("/", axum::routing::get(http::list_devices).post(http::register_device))
        .route("/:id", axum::routing::get(http::get_device))
        .route("/:id/status", axum::routing::put(http::update_device_status))
}

fn bandwidth_routes() -> Router<SharedState> {
    use crate::modules::bandwidth::api::http;
    Router::new()
        .route("/profiles", axum::routing::get(http::list_profiles).post(http::create_profile))
        .route("/profiles/:id", axum::routing::put(http::update_profile).delete(http::delete_profile))
}

fn ticket_routes() -> Router<SharedState> {
    use crate::modules::ticket::api::http;
    Router::new()
        .route("/", axum::routing::get(http::list_tickets).post(http::create_ticket))
...
fn notification_routes() -> Router<SharedState> {
    ...
    Router::new()
        .route("/templates", axum::routing::get(http::list_templates).post(http::create_template))
...
fn audit_routes() -> Router<SharedState> {
    ...
    Router::new()
        .route("/logs", axum::routing::get(http::list_audit_logs))
}

fn lead_routes() -> Router<SharedState> {
    use crate::modules::lead::api::http;
    Router::new()
        .route("/", axum::routing::get(http::list_leads).post(http::create_lead))
...
fn referral_routes() -> Router<SharedState> {
    ...
    Router::new()
        .route("/", axum::routing::get(http::list_referrals).post(http::create_referral))
...
fn coverage_routes() -> Router<SharedState> {
    ...
    Router::new()
        .route("/areas", axum::routing::get(http::list_coverage_areas).post(http::create_coverage_area))
...
fn discovery_routes() -> Router<SharedState> {
    ...
    Router::new()
        .route("/scans", axum::routing::get(http::list_scans).post(http::create_scan))
        .route("/results", axum::routing::get(http::list_results))
        .route("/results/:id/approve", axum::routing::post(http::approve_result))
}

fn inventory_routes() -> Router<SharedState> {
    use crate::modules::inventory::api::http;
    Router::new()
        .route("/", axum::routing::get(http::list_inventory).post(http::create_inventory_item))
        .route("/:id/assign", axum::routing::post(http::assign_inventory_item))
}

fn installation_routes() -> Router<SharedState> {
    use crate::modules::installation::api::http;
    Router::new()
        .route("/", axum::routing::get(http::list_installations).post(http::create_installation))
