//! Branch-scoping middleware — injects branch filter into request context.
...
use crate::shared::types::{BranchFilter, UserContext};

/// Company-wide roles that bypass branch scoping.
const COMPANY_WIDE_ROLES: &[&str] = &[
    "super_admin",
    "isp_owner",
    "finance_manager",
];
...
/// Extract `BranchFilter` from request extensions (for use in handlers).
pub fn extract_branch_filter(req: &Request) -> Option<BranchFilter> {
    req.extensions().get::<BranchFilter>().cloned()
}
