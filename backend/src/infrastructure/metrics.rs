...
impl Metrics {
    pub fn new() -> Self {
        ...
        let http_request_duration_seconds = prometheus::Histogram::with_opts(
            prometheus::histogram_opts!("aeroxe_http_request_duration_seconds", "Request latency in seconds")
                .buckets(vec![0.01, 0.025, 0.05, 0.1, 0.25, 0.5, 1.0, 2.0, 5.0])
        ).unwrap();
        let db_connections_active = IntGauge::with_opts(
            prometheus::opts!("aeroxe_db_connections_active", "Active DB connections"),
        ).unwrap();
        let active_subscriptions = IntGauge::with_opts(
            prometheus::opts!("aeroxe_active_subscriptions", "Current active subscriptions"),
        ).unwrap();
        let invoices_generated_total = IntCounter::with_opts(
            prometheus::opts!("aeroxe_invoices_generated_total", "Total invoices generated"),
        ).unwrap();
        let device_online_count = IntGauge::with_opts(
            prometheus::opts!("aeroxe_device_online_count", "Online device count"),
        ).unwrap();
        let revenue_total = Gauge::with_opts(
            prometheus::opts!("aeroxe_revenue_total", "Total revenue in INR"),
        ).unwrap();
        let worker_cycles_total = IntCounter::with_opts(
            prometheus::opts!("aeroxe_worker_cycles_total", "Total worker cycles"),
        ).unwrap();
        let worker_errors_total = IntCounter::with_opts(
            prometheus::opts!("aeroxe_worker_errors_total", "Worker cycle errors"),
        ).unwrap();
        let nats_messages_published = IntCounter::with_opts(
            prometheus::opts!("aeroxe_nats_messages_published", "NATS messages published"),
        ).unwrap();
        let nats_messages_consumed = IntCounter::with_opts(
            prometheus::opts!("aeroxe_nats_messages_consumed", "NATS messages consumed"),
        ).unwrap();
        ...
        registry.register(Box::new(invoices_generated_total.clone())).unwrap();
        registry.register(Box::new(device_online_count.clone())).unwrap();
        registry.register(Box::new(revenue_total.clone())).unwrap();
        registry.register(Box::new(worker_cycles_total.clone())).unwrap();
        registry.register(Box::new(worker_errors_total.clone())).unwrap();
        registry.register(Box::new(nats_messages_published.clone())).unwrap();
        registry.register(Box::new(nats_messages_consumed.clone())).unwrap();
