...
use crate::modules::payment_gateway::service::payment_gateway_service_seaorm::PaymentGatewayServiceSeaorm;

pub async fn list_gateways(State(state): State<SharedState>) -> Result<Json<Vec<GatewayConfigResponse>>, AppError> {
    let svc = PaymentGatewayServiceSeaorm::new(&state.db_seaorm);
