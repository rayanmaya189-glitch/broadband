//! SeaORM-based controller for the Coverage domain.
...
use crate::modules::coverage::service::coverage_service_seaorm::CoverageServiceSeaorm;
...
pub async fn list_areas(State(state): State<SharedState>, Query(q): Query<CoverageQuery>) -> Result<Json<Vec<CoverageAreaResponse>>, AppError> {
    let svc = CoverageServiceSeaorm::new(&state.db_seaorm);
