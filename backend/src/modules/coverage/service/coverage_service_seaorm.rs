//! SeaORM-based service for the Coverage domain.

use sea_orm::DatabaseConnection;

use crate::common::errors::app_error::AppError;
use crate::modules::coverage::repository::coverage_repository_seaorm::CoverageRepositorySeaorm;
use crate::modules::coverage::request::coverage_request::*;
use crate::modules::coverage::response::coverage_response::*;

pub struct CoverageServiceSeaorm<'a> {
    repo: CoverageRepositorySeaorm<'a>,
}

impl<'a> CoverageServiceSeaorm<'a> {
    pub fn new(db: &'a DatabaseConnection) -> Self {
        Self { repo: CoverageRepositorySeaorm::new(db) }
    }
    ...
    pub async fn get_area(&self, id: i64) -> Result<CoverageAreaResponse, AppError> {
        ...
        Ok(CoverageAreaResponse { id: a.id, branch_id: a.branch_id, name: a.name, description: a.description, area_type: a.area_type, is_active: a.is_active, fiber_available: a.fiber_available, estimated_installation_days: a.estimated_installation_days, current_customers: a.current_customers, created_at: a.created_at.into() })
    }
    ...
    pub async fn update_area(&self, id: i64, req: UpdateCoverageAreaRequest) -> Result<CoverageAreaResponse, AppError> {
        let a = self.repo.update(id, req.name.as_deref(), req.description.as_deref(), req.area_type.as_deref(), req.fiber_available, req.estimated_installation_days, req.max_customers).await?;
        Ok(CoverageAreaResponse { id: a.id, branch_id: a.branch_id, name: a.name, description: a.description, area_type: a.area_type, is_active: a.is_active, fiber_available: a.fiber_available, estimated_installation_days: a.estimated_installation_days, current_customers: a.current_customers, created_at: a.created_at.into() })
    }
    ...
    pub async fn list_pincodes(&self, area_id: i64) -> Result<Vec<CoveragePincodeResponse>, AppError> {
        ...
        Ok(pincodes.into_iter().map(|p| CoveragePincodeResponse { id: p.id, pincode: p.pincode, city: p.city, district: p.district, state: p.state, is_active: p.is_active, created_at: p.created_at.into() }).collect())
    }
    ...
    pub async fn add_pincode(&self, area_id: i64, req: AddPincodeRequest) -> Result<CoveragePincodeResponse, AppError> {
        ...
        Ok(CoveragePincodeResponse { id: p.id, pincode: p.pincode, city: p.city, district: p.district, state: p.state, is_active: p.is_active, created_at: p.created_at.into() })
    }
