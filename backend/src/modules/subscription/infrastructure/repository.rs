...
impl<'a> SubscriptionRepository<'a> {
    ...
    pub async fn find_by_id(&self, id: i64) -> Result<Option<<Subscription as sea_orm::EntityTrait>::Model>, AppError> {
        Ok(Subscription::find_by_id(id).one(self.db).await?)
    }

    pub async fn find_by_customer(&self, customer_id: i64) -> Result<Vec<<Subscription as sea_orm::EntityTrait>::Model>, AppError> {
