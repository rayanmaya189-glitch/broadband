...
pub struct BandwidthService<'a> {
    repo: BandwidthRepository<'a>,
}

impl<'a> BandwidthService<'a> {
    pub fn new(db: &'a DatabaseConnection) -> Self {
        Self {
            repo: BandwidthRepository::new(db),
        }
    }
    ...
    pub async fn list_profiles(&self, page: i64, per_page: i64) -> Result<BandwidthProfileListResponse, AppError> {
        ...
        let responses: Vec<BandwidthProfileResponse> = profiles.into_iter().map(|p| BandwidthProfileResponse {
            id: p.id,
            name: p.name,
            ...
            burst_duration_seconds: p.burst_duration_seconds,
            priority: p.priority,
    ...
    pub async fn create_profile(&self, req: CreateBandwidthProfileRequest) -> Result<BandwidthProfileResponse, AppError> {
        let p = self.repo.create(
            &req.name, req.description.as_deref(), req.plan_id,
            req.download_kbps, req.upload_kbps,
            req.burst_download_kbps, req.burst_upload_kbps,
            req.burst_duration_seconds, req.priority,
        ).await?;
        Ok(BandwidthProfileResponse {
            id: p.id,
            name: p.name,
    ...
    pub async fn apply_to_subscription(&self, profile_id: i64, req: ApplyProfileRequest) -> Result<BandwidthApplicationResponse, AppError> {
        ...
        Ok(BandwidthApplicationResponse {
            id: app.id,
            profile_id: app.profile_id,
            subscription_id: app.subscription_id,
            device_id: app.device_id,
            status: app.status,
            applied_at: app.applied_at.map(|v| v.into()),
    ...
    pub async fn list_applications(&self, profile_id: Option<i64>, page: i64, per_page: i64) -> Result<Vec<BandwidthApplicationResponse>, AppError> {
        ...
        Ok(apps.into_iter().map(|a| BandwidthApplicationResponse {
            id: a.id,
            profile_id: a.profile_id,
            subscription_id: a.subscription_id,
            device_id: a.device_id,
            status: a.status,
            applied_at: a.applied_at.map(|v| v.into()),
    ...
    pub async fn get_usage(&self, subscription_id: i64, page: i64, per_page: i64) -> Result<BandwidthUsageResponse, AppError> {
        ...
        Ok(BandwidthUsageResponse {
            ...
            records: records.into_iter().map(|r| BandwidthUsageRecord {
                id: r.id,
                download_bytes: r.download_bytes,
                upload_bytes: r.upload_bytes,
                recorded_at: r.recorded_at.into(),
            }).collect(),
        })
    }
}
