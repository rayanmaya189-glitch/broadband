...
pub struct CustomerRepositorySeaorm {
    db: DatabaseConnection,
}

impl CustomerRepositorySeaorm {
    ...
    pub async fn create(
        ...
        let model = active.insert(&self.db).await.map_err(AppError::DatabaseSeaorm)?;
        Ok(model)
    }
    ...
    pub async fn update(
        ...
        let updated = active.update(&self.db).await.map_err(AppError::DatabaseSeaorm)?;
        Ok(updated)
    }
    ...
    pub async fn list(
        ...
        let total = paginator.num_items().await.map_err(AppError::DatabaseSeaorm)? as i64;
        let models = paginator.fetch_page(page_num - 1).await.map_err(AppError::DatabaseSeaorm)?;
    ...
    pub async fn upsert_profile(
        ...
        if let Some(model) = existing {
            ...
            let inserted = active.insert(&self.db).await.map_err(AppError::DatabaseSeaorm)?;
            Ok(inserted)
        }
    }
