...
impl<'a> PlanService<'a> {
    ...
    pub async fn publish_plan(&self, id: i64) -> Result<PlanDetailResponse, AppError> {
        ...
        let p = self.repo.publish(id, None).await?;
        self.cache.invalidate_by_id(id).await.ok();
        Ok(plan_to_response(&p))
    }
    ...
    pub async fn unpublish_plan(&self, id: i64) -> Result<PlanDetailResponse, AppError> {
        let p = self.repo.find_by_id(id).await?.ok_or_else(|| AppError::NotFound("Plan not found".into()))?;
        if !p.is_active {
            return Err(AppError::Validation("Plan is already unpublished".into()));
        }
    ...
    pub async fn clone_plan(&self, id: i64) -> Result<PlanCloneResponse, AppError> {
        let original = self.repo.find_by_id(id).await?.ok_or_else(|| AppError::NotFound("Plan not found".into()))?;
        let new_plan = self.repo.clone(id).await?;
        self.cache.invalidate_prefix().await.ok();
        Ok(PlanCloneResponse {
            original_plan_id: original.id,
            new_plan_id: new_plan.id,
            new_plan_code: new_plan.code,
            message: format!("Plan '{}' cloned successfully", original.name),
        })
    }
    ...
    pub async fn get_speed_profile(&self, plan_id: i64) -> Result<SpeedProfileResponse, AppError> {
        self.repo.find_by_id(plan_id).await?.ok_or_else(|| AppError::NotFound("Plan not found".into()))?;
        let sp = self.repo.get_speed_profile(plan_id).await?.ok_or_else(|| AppError::NotFound("No speed profile found for this plan".into()))?;
        Ok(SpeedProfileResponse {
            id: sp.id, plan_id: sp.plan_id, name: sp.name,
            download_limit_kbps: sp.download_limit_kbps, upload_limit_kbps: sp.upload_limit_kbps,
            burst_download_kbps: sp.burst_download_kbps, burst_upload_kbps: sp.burst_upload_kbps,
            burst_duration_seconds: sp.burst_duration_seconds, priority_queue: sp.priority_queue,
            qos_marking: sp.qos_marking, fq_codel_enabled: sp.fq_codel_enabled,
            device_type: sp.device_type, created_at: sp.created_at, updated_at: sp.updated_at,
        })
    }

    pub async fn create_speed_profile(&self, plan_id: i64, req: &CreateSpeedProfileRequest) -> Result<SpeedProfileResponse, AppError> {
        self.repo.find_by_id(plan_id).await?.ok_or_else(|| AppError::NotFound("Plan not found".into()))?;
        let sp = self.repo.upsert_speed_profile(
            plan_id, &req.name, req.download_limit_kbps, req.upload_limit_kbps,
            req.burst_download_kbps, req.burst_upload_kbps, req.burst_duration_seconds,
            req.priority_queue, req.qos_marking.as_deref(), req.htb_parent_queue.as_deref(),
            req.fq_codel_enabled, req.device_type.as_deref(),
        ).await?;
    ...
    pub async fn delete_speed_profile(&self, plan_id: i64) -> Result<MessageResponse, AppError> {
        ...
        self.cache.invalidate_by_id(plan_id).await.ok();
        Ok(MessageResponse { message: "Speed profile deleted".into() })
    }

    // ── Plan Pricing ────────────────────────────────────────

    pub async fn list_pricing(&self, plan_id: i64) -> Result<Vec<PlanPricingResponse>, AppError> {
        self.repo.find_by_id(plan_id).await?.ok_or_else(|| AppError::NotFound("Plan not found".into()))?;
        let pricing = self.repo.list_pricing(plan_id).await?;
        Ok(pricing.into_iter().map(|p| PlanPricingResponse {
            id: p.id, plan_id: p.plan_id, billing_period_months: p.billing_period_months,
            price: p.price, savings_amount: p.savings_amount, savings_percent: p.savings_percent,
            is_active: p.is_active, created_at: p.created_at,
        }).collect())
    }

    pub async fn update_pricing(&self, plan_id: i64, req: &UpdatePlanPricingRequest) -> Result<PlanPricingResponse, AppError> {
        self.repo.find_by_id(plan_id).await?.ok_or_else(|| AppError::NotFound("Plan not found".into()))?;
        if !(1..=12).contains(&req.billing_period_months) {
            return Err(AppError::Validation("Billing period must be 1-12 months".into()));
        }
        let p = self.repo.upsert_pricing(plan_id, req.billing_period_months, req.price).await?;
        self.cache.invalidate_by_id(plan_id).await.ok();
        Ok(PlanPricingResponse {
            id: p.id, plan_id: p.plan_id, billing_period_months: p.billing_period_months,
            price: p.price, savings_amount: p.savings_amount, savings_percent: p.savings_percent,
            is_active: p.is_active, created_at: p.created_at,
        })
