...
//!
//! Uses `PlanRepositorySeaorm` which operates on `DatabaseConnection`
...
use crate::modules::plan::repository::plan_repository_seaorm::PlanRepositorySeaorm;
...
pub struct PlanServiceSeaorm<'a> {
    repo: PlanRepositorySeaorm<'a>,
    cache: CacheHelper<'a>,
}

impl<'a> PlanServiceSeaorm<'a> {
    pub fn new(db: &'a DatabaseConnection, redis: &'a RedisService) -> Self {
        Self {
            repo: PlanRepositorySeaorm::new(db),
