...
//!
//! Uses `state.db_seaorm` (SeaORM `DatabaseConnection`) instead of `state.db` (sqlx `PgPool`).
...
use crate::modules::plan::service::plan_service_seaorm::PlanServiceSeaorm;
...
pub async fn list_plans(
    ...
    let svc = PlanServiceSeaorm::new(&state.db_seaorm, &state.redis);
