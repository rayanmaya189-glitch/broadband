//! SeaORM-based controller for the Permission domain.
...
use crate::modules::permission::service::permission_service_seaorm::PermissionServiceSeaorm;

pub async fn list_permissions(
    State(state): State<SharedState>,
    Query(query): Query<ListPermissionsQuery>,
) -> Result<Json<PaginatedResponse<PermissionResponse>>, AppError> {
    let svc = PermissionServiceSeaorm::new(&state.db_seaorm);
