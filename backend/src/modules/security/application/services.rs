...
use crate::modules::security::domain::entities::{Role, Permission, UserRole, RolePermission, RoleColumn, UserRoleColumn, RolePermissionColumn, UserRoleActiveModel};

pub struct SecurityService;

impl SecurityService {
    pub async fn resolve_permissions(db: &DatabaseConnection, user_id: i64) -> Result<Vec<String>, AppError> {
    ...
    pub async fn has_permission(db: &DatabaseConnection, user_id: i64, permission: &str) -> Result<bool, AppError> {
        let permissions = Self::resolve_permissions(db, user_id).await?;
        Ok(permissions.contains(&permission.to_string()))
    }

    pub async fn list_roles(db: &DatabaseConnection) -> Result<Vec<crate::modules::security::domain::entities::role::Model>, AppError> {
        let roles = Role::find().filter(RoleColumn::IsActive.eq(true)).all(db).await?;
        Ok(roles)
    }

    pub async fn list_permissions(db: &DatabaseConnection) -> Result<Vec<crate::modules::security::domain::entities::permission::Model>, AppError> {
    ...
    pub async fn assign_role(db: &DatabaseConnection, user_id: i64, role_id: i64, assigned_by: Option<i64>) -> Result<(), AppError> {
        ...
        if let Some(ur) = existing {
            ...
            let new_ur = UserRoleActiveModel {
                user_id: Set(user_id), role_id: Set(role_id), assigned_by: Set(assigned_by),
                is_active: Set(true), created_at: Set(chrono::Utc::now()), ..Default::default()
            };
