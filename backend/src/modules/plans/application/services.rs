...
use crate::modules::plans::domain::entities::{Plan, PlanPricing, SpeedProfile, PlanColumn, PlanPricingColumn, SpeedProfileColumn, PlanActiveModel, PlanPricingActiveModel, SpeedProfileActiveModel};

pub struct PlanService;

impl PlanService {
    pub async fn list_active_plans(db: &DatabaseConnection) -> Result<Vec<crate::modules::plans::domain::entities::plan::Model>, AppError> {
        let plans = Plan::find()
    ...
    ) -> Result<(crate::modules::plans::domain::entities::plan::Model, Vec<crate::modules::plans::domain::entities::plan_pricing::Model>), AppError> {
        let plan = Plan::find_by_id(id).one(db).await?
    ...
    pub async fn create_plan(
        db: &DatabaseConnection, slug: String, name: String, description: Option<String>,
        speed_label: String, download_mbps: i32, upload_mbps: i32,
        burst_mbps: Option<i32>, is_business: bool,
    ) -> Result<crate::modules::plans::domain::entities::plan::Model, AppError> {
        let now = chrono::Utc::now();
        let new_plan = PlanActiveModel {
            slug: Set(slug), name: Set(name), description: Set(description),
            speed_label: Set(speed_label), download_mbps: Set(download_mbps),
            upload_mbps: Set(upload_mbps), burst_mbps: Set(burst_mbps),
            is_business: Set(is_business), is_active: Set(true),
    ...
    pub async fn update_pricing(
        db: &DatabaseConnection, plan_id: i64, billing_period_months: i32,
        ...
        if let Some(p) = existing {
            ...
            let new_pricing = PlanPricingActiveModel {
                plan_id: Set(plan_id), billing_period_months: Set(billing_period_months),
                price: Set(price), is_active: Set(true),
                created_at: Set(chrono::Utc::now()), ..Default::default()
            };
    ...
    pub async fn upsert_speed_profile(
        db: &DatabaseConnection, plan_id: i64, name: String,
        download_limit_kbps: i32, upload_limit_kbps: i32, device_type: String,
    ) -> Result<crate::modules::plans::domain::entities::speed_profile::Model, AppError> {
        let existing = SpeedProfile::find().filter(SpeedProfileColumn::PlanId.eq(plan_id)).one(db).await?;
        if let Some(sp) = existing {
            ...
            let new_sp = SpeedProfileActiveModel {
                plan_id: Set(plan_id), name: Set(name),
                download_limit_kbps: Set(download_limit_kbps),
                upload_limit_kbps: Set(upload_limit_kbps),
                device_type: Set(device_type), fq_codel_enabled: Set(Some(true)),
    ...
    pub async fn list_all_plans(db: &DatabaseConnection) -> Result<Vec<crate::modules::plans::domain::entities::plan::Model>, AppError> {
        let plans = Plan::find().all(db).await?;
        Ok(plans)
    }

    pub async fn approve_plan(db: &DatabaseConnection, id: i64, approved_by: i64) -> Result<crate::modules::plans::domain::entities::plan::Model, AppError> {
        let plan = Plan::find_by_id(id).one(db).await?
            .ok_or_else(|| AppError::NotFound(format!("Plan {} not found", id)))?;
